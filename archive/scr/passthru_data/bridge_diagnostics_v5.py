"""Projection-based diagnostics for the CIF-based package/raw outcome bridge."""
...
def _root(config: PipelineConfig) -> Path:
    v3 = config.verification_dir / "trade_regressions" / "package_benchmark_v5" / "common_sample_v5_cif"
    if (v3 / "package_common_sample_anchor.parquet").exists() and (v3 / "raw_outcomes_package_policy_cif.parquet").exists():
...
def run_bridge_diagnostics(config: PipelineConfig) -> dict[str, object]:
    ...
    v5_base = base / "common_sample_v5_cif"
    package = v5_base / "package_common_sample_anchor.parquet"
    raw = v5_base / "raw_outcomes_package_policy_cif.parquet"
    ...
    bridge_root = base / "common_sample_v5_cif" / "bridge_resumable_v5"
    ...
    for mode in ("package_common_sample_anchor", "raw_outcomes_package_policy"):
        for spec in ("event", "dynamic"):
            for outcome in OUTCOMES:
                path = bridge_root / mode / spec / outcome / "coefficients.parquet" if bridge_root.name == "bridge_resumable_v5" else bridge_root / mode / spec / f"{outcome}.parquet"
                if path.exists():
    ...
    for spec in ("event", "dynamic"):
        for outcome in OUTCOMES:
            left = read_table(bridge_root / "package_common_sample_anchor" / spec / outcome / "coefficients.parquet" if bridge_root.name == "bridge_resumable_v5" else bridge_root / "package_common_sample_anchor" / spec / f"{outcome}.parquet").rename(columns={"event_time": "horizon"})
            right = read_table(bridge_root / "raw_outcomes_package_policy" / spec / outcome / "coefficients.parquet" if bridge_root.name == "bridge_resumable_v5" else bridge_root / "raw_outcomes_package_policy" / spec / f"{outcome}.parquet").rename(columns={"event_time": "horizon"})
