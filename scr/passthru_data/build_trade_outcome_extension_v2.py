...
VERSION = "raw_trade_outcome_extension_v2_cif"
...
def build(config: PipelineConfig, *, overwrite: bool = False) -> dict[str, Any]:
    source_root = config.analysis_dir / "extension_v4_cif" / "flow=imports"
    destination_root = config.analysis_dir / "extension_outcomes_v2_cif" / "flow=imports"
    verification_root = config.verification_dir / "extension_outcomes_v2_cif"
        ...
        for source in source_files:
            ...
            if not destination.exists() or overwrite:
                ...
                query = f"""
                  SELECT flow, partner_code, partner_name, hs10, hs8, hs6, hs4, hs2,
                         year, month, period, gen_val_mo, gen_cif_mo, gen_qy1_mo,
                         dut_val_mo, cal_dut_mo, quantity_missing, quantity_zero,
                         CASE WHEN gen_cif_mo > 0 AND gen_qy1_mo > 0 THEN gen_cif_mo::DOUBLE / gen_qy1_mo END AS pre_duty_unit_value,
                         CASE WHEN gen_cif_mo > 0 AND gen_qy1_mo > 0 AND cal_dut_mo IS NOT NULL AND cal_dut_mo >= 0
                              THEN (gen_cif_mo::DOUBLE + cal_dut_mo) / gen_qy1_mo END AS duty_inclusive_unit_value,
            ...
            stats = con.execute(f"""
              SELECT count(*) AS rows, count(DISTINCT (partner_code,hs10,year,month)) AS distinct_keys,
                     sum(gen_cif_mo)::DOUBLE AS constructed_value,
    ...
    build_manifest = {
        ...
        "source_kind": "archive_native_extension_v4_cif_import_partitions",
