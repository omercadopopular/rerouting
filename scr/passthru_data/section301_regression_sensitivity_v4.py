"""Section 301 regression sensitivity v4 with family-correct policy bridges."""
...
SENSITIVITY_VERSION = "v4"
LEGACY_INVALID_VERSION = "v3"
...
def _legacy_status_paths(config: PipelineConfig) -> tuple[Path, Path]:
    v3_dir = config.verification_dir / "raw_replication_imports" / LEGACY_INVALID_VERSION
    v3_dir.mkdir(parents=True, exist_ok=True)
    return v3_dir / "status.json", config.verification_dir / "raw_replication_imports" / "section301_regression_sensitivity_v2_status.json"
...
def _write_invalid_legacy_status(config: PipelineConfig) -> None:
    ...
    payload = {
        ...
        "valid": False,
        "reason": "v3 diagnostic-only: incorrect pooled target; missing family-specific bridge filter; missing cross-family exclusion; non-990388 raw treatment leakage; non-family-specific dates; package original p/pduty unused; unreliable provenance classification",
    }
    write_metadata_json(v2_status_path, payload)
    write_metadata_json(v1_status_path, {**payload, "version": "v2"})
...
def _period_bounds(frame: pd.DataFrame) -> tuple[str | None, str | None]:
    """Return lexicographic bounds from complete year-month pairs."""
    if frame.empty or not {"year", "month"}.issubset(frame.columns):
        return None, None
    pairs = pd.to_numeric(frame["year"], errors="coerce").astype("Int64").astype("string") + "-" + pd.to_numeric(frame["month"], errors="coerce").astype("Int64").astype("string").str.zfill(2)
    pairs = pairs.dropna()
    return (str(pairs.min()), str(pairs.max())) if not pairs.empty else (None, None)
...
def _build_package_paper_window_cache(config: PipelineConfig, overwrite: bool = False) -> tuple[Path, dict[str, Any]]:
    ...
    try:
        for chunk in chunk_reader:
            ...
            current_min, current_max = _period_bounds(chunk)
...
def _build_raw_paper_window_cache(config: PipelineConfig, overwrite: bool = False) -> tuple[Path, dict[str, Any]]:
    ...
    query = f"""
        ...
        WHERE cty_code > 0
          AND year >= 2017
          AND (year < 2019 OR (year = 2019 AND month <= 4))
    """
    ...
    metadata = {
        **_source_metadata(source),
        ...
        "period_min": _period_bounds(frame)[0],
        "period_max": _period_bounds(frame)[1],
...
def _write_preflight(config: PipelineConfig, package_meta: dict[str, Any], raw_meta: dict[str, Any], package_path: Path, raw_path: Path) -> tuple[Path, Path, list[dict[str, Any]]]:
    ...
    for role, path, meta, stat in (
        ...
        if exists and path.suffix.lower() == ".parquet":
            ...
            # Do not read the two-gigabyte benchmark merely to obtain a row count.
            # The filtered cache metadata is the actual input cardinality used by v4.
            row_count = int(meta.get("rows", 0)) or None
...
def _build_master_panel(config: PipelineConfig, package_path: Path, raw_path: Path) -> pd.DataFrame:
    ...
    master["pkg_m_p_original"] = pd.to_numeric(master["pkg_m_p"], errors="coerce")
    master["pkg_m_pduty_original"] = pd.to_numeric(master["pkg_m_pduty"], errors="coerce")
    ...
    master["pkg_pair_target_all"] = (
        master.groupby("id", sort=False)["pkg_m_status2"].transform(lambda s: s.eq(2).any())
    ).astype("int8")
    raw_301_row = (
        pd.to_numeric(master["raw_tw_increment_rate_raw"], errors="coerce").fillna(0).gt(0)
        & master["raw_tw_rule_code_raw"].astype("string").str.replace(r"\D", "", regex=True).str.startswith("990388", na=False)
    )
    raw_301_pair = raw_301_row.groupby(master["id"], sort=False).transform("any")
    master["raw_pair_target"] = (master["cty_code"].eq(5700) & raw_301_pair).astype("int8")
    master["pkg_pair_target_all"] = master["pkg_pair_target_all"].astype("int8")
    pkg_china_active = master["pkg_m_status2"].eq(2) & master["pkg_m_china_hit"].eq(1)
    first_pkg = master.loc[master["pkg_m_status2"].eq(2)].groupby("id")["mdate_index"].min()
    first_pkg_china = master.loc[pkg_china_active].groupby("id")["mdate_index"].min()
    master["pkg_first_active_mdate2"] = master["id"].map(first_pkg).astype("Int64")
    master["pkg_first_china_active_mdate2"] = master["id"].map(first_pkg_china).astype("Int64")
    ...
    pkg_cross = (
        master[["pkg_m_alum_hit", "pkg_m_steel_hit", "pkg_m_washer_hit", "pkg_m_solar_hit"]]
        .fillna(0).astype(int).groupby(master["id"], sort=False).max().max(axis=1)
    )
    master["pkg_cross_family_pair"] = master["id"].map(pkg_cross).fillna(0).astype("int8")
    raw_code_digits = master["raw_tw_rule_code_raw"].astype("string").str.replace(r"\D", "", regex=True)
    master["raw_cross_family_pair"] = (
        raw_code_digits.str.startswith(("990380", "990385", "990345", "990346"), na=False)
        .groupby(master["id"], sort=False).transform("any").astype("int8")
    )
    return master.sort_values(KEY_COLUMNS).reset_index(drop=True)


def _calendar_index(master: pd.DataFrame, variant: VariantSpec, analysis: str | None = None) -> pd.Series:
    family_specific = analysis == "china301_policy_bridge"
    if variant.calendar_source == "pkg_legal":
        fill_source = "pkg_first_china_active_mdate2" if family_specific else "pkg_first_active_mdate2"
...
def _target_column(master: pd.DataFrame, variant: VariantSpec, analysis: str | None = None) -> pd.Series:
    if variant.target_source == "pkg":
        if analysis == "pooled_outcome_bridge":
            return master["pkg_pair_target_all"].astype("int8")
...
def _outcome_column(master: pd.DataFrame, variant: VariantSpec, outcome: str) -> pd.Series:
    ...
    if source == "pkg" and outcome == "p":
        return pd.to_numeric(master["pkg_m_p_original"], errors="coerce")
    if source == "pkg" and outcome == "pduty":
        return pd.to_numeric(master["pkg_m_pduty_original"], errors="coerce")
...
def _prepare_event_frame(master: pd.DataFrame, variant: VariantSpec, window: str, outcome: str, analysis: str | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
    spec = WINDOWS[window]
    work = master.copy()
    work["T"] = _target_column(work, variant, analysis=analysis)
    work["d_index"] = _calendar_index(work, variant, analysis=analysis)
    for sector_col in ("naics4", "naics3", "naics2"):
    ...
    if outcome == "val":
        ...
        outcome_values = _outcome_column(work, variant, outcome) if variant.outcome_source == "pkg" else base_price
    elif outcome == "pduty":
        outcome_values = _outcome_column(work, variant, outcome) if variant.outcome_source == "pkg" else base_price * (1.0 + tariff_rate.fillna(0.0))
    else:
...
def _bridge_master(master: pd.DataFrame, bridge: str) -> pd.DataFrame:
    """Apply the family-specific comparison universe before event construction."""
    if bridge == "pooled_outcome_bridge":
        return master.copy()
    target_hs10 = master.loc[
        master["pkg_pair_target"].eq(1) | master["raw_pair_target"].eq(1), "hs10"
    ].dropna().astype("string").unique()
    in_union = master["hs10"].astype("string").isin(target_hs10)
    cross_family = master["pkg_cross_family_pair"].eq(1) | master["raw_cross_family_pair"].eq(1)
    return master.loc[in_union & ~cross_family].copy()
...
def _write_provenance_audit(config: PipelineConfig, artifact_dir: Path) -> Path:
    """Persist source-level evidence without manufacturing structural support."""
    rows: list[pd.DataFrame] = []
    for filename, source_kind, support_class in (
        ("tradewar_machine_links.parquet", "machine", "explicit_machine_link"),
        ("tradewar_pdf_links.parquet", "persisted_pdf", "persisted_pdf_link_unverified"),
    ):
        ...
        if frame.empty:
            continue
        for column in ("release_name", "release_start_date", "release_end_date", "hs8", "rule_code"):
            if column not in frame.columns:
                frame[column] = pd.NA
        frame["hs8"] = frame["hs8"].map(lambda value: normalize_hs_code(value, 8)).astype("string")
        frame["candidate_rule_code"] = frame["rule_code"].map(lambda value: normalize_hs_code(value, 8)).astype("string")
        frame["policy_family"] = frame["candidate_rule_code"].map(lambda value: "section_301" if str(value).startswith("990388") else ("section_232_steel" if str(value).startswith("990380") else ("section_232_aluminum" if str(value).startswith("990385") else ("section_201" if str(value).startswith(("990345", "990346")) else "other"))))
        frame["source_kind"] = source_kind
        frame["official_source_support_class"] = support_class
        frame["same_structural_note_block"] = False
        frame["rule_found_in_same_row"] = False
        frame["rule_found_only_in_context"] = False
        frame["cross_family_candidate"] = frame["policy_family"].ne("section_301")
        frame["source_file"] = filename
        frame["source_page"] = pd.NA
        frame["source_row"] = pd.NA
        frame["extraction_method"] = source_kind
        rows.append(frame[["hs8", "candidate_rule_code", "policy_family", "release_name", "release_start_date", "source_file", "source_page", "source_row", "extraction_method", "rule_found_in_same_row", "rule_found_only_in_context", "same_structural_note_block", "cross_family_candidate", "official_source_support_class", "source_kind"]])
    out = pd.concat(rows, ignore_index=True).drop_duplicates() if rows else pd.DataFrame()
    path = artifact_dir / "section301_source_provenance.parquet"
    write_parquet(out, path, overwrite=True)
    if not out.empty:
        out.groupby(["source_kind", "policy_family", "official_source_support_class"], dropna=False).size().reset_index(name="rows").to_csv(artifact_dir / "section301_source_provenance_summary.csv", index=False)
    return path
...
def _build_diagnostics(master: pd.DataFrame, artifact_dir: Path) -> dict[str, Any]:
    ...
    product_map = china[["hs10", "pkg_pair_target", "raw_pair_target", "pkg_cross_family_pair", "raw_cross_family_pair"]].drop_duplicates("hs10")
    write_parquet(product_map, artifact_dir / "section301_treatment_map_products.parquet", overwrite=True)
    return {
...
def run_section301_regression_sensitivity(config: PipelineConfig) -> dict[str, Any]:
    ...
    diagnostics["source_provenance_path"] = str(provenance_path)
    ...
    for bridge in BRIDGES:
        bridge_variants = _bridge_variants(bridge)
        bridge_master = _bridge_master(master, bridge)
        for window in WINDOWS:
            for outcome in OUTCOMES:
                ...
                for variant in bridge_variants:
                    ...
                    if source_code is not None and source_code in prepared_by_variant:
                        ...
                        continue
                    prepared, _ = _prepare_event_frame(bridge_master, variant, window, outcome, analysis=bridge)
                    prepared_by_variant[variant.code] = prepared
                    frames_by_variant[variant.code] = prepared
                common = _build_common_sample_keys(bridge_master, bridge, window, outcome, frames_by_variant)
