...
Version 3 remains historical.  Version 4 is deliberately a new namespace so
...
def root(config: PipelineConfig) -> Path:
    path = config.verification_dir / "trade_regressions" / "package_benchmark_v5" / "common_sample_v4"
...
def build_source_separated_panels(config: PipelineConfig, *, overwrite: bool = False) -> dict[str, Any]:
    ...
    extension_root = config.analysis_dir / "extension_v2"
    ...
    raw_out = out / "raw_outcomes_package_policy_realized_duty.parquet"
    p = _sql_path(package)
    ...
    """
    package_values = """
        CASE WHEN p.m_val > 0 AND r.trade_value > 0 THEN p.m_val END AS m_val,
        CASE WHEN p.m_q1 > 0 AND r.quantity > 0 THEN p.m_q1 END AS m_q1,
        CASE WHEN p.m_p > 0 AND r.trade_value > 0 AND r.quantity > 0 THEN p.m_p END AS m_p,
    ...
    """
    raw_values = """
        CASE WHEN r.trade_value > 0 THEN r.trade_value / 1000000.0 END AS m_val,
        CASE WHEN r.quantity > 0 THEN r.quantity / 1000000.0 END AS m_q1,
        CASE WHEN r.trade_value > 0 AND r.quantity > 0 THEN r.trade_value::DOUBLE / r.quantity END AS m_p,
        CASE WHEN r.trade_value > 0 AND r.quantity > 0 AND r.cal_dut_mo IS NOT NULL
                  AND r.cal_dut_mo >= 0 THEN (r.trade_value::DOUBLE + r.cal_dut_mo) / r.quantity END AS m_pduty,
        r.trade_value, r.quantity, r.quantity_missing, r.quantity_zero,
        r.dut_val_mo, r.cal_dut_mo, r.source_archive, r.source_member,
    ...
    manifest = {
        ...
        "raw_outcome_formula": "(trade_value + cal_dut_mo) / quantity",
