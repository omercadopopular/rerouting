...
def policy_root(config: PipelineConfig) -> Path:
    """Canonical independently reconstructed tariff artifacts."""
    return config.repo_root / "data" / "processed" / "tariffs" / "final"

VERSION = "historical_replication_locked_v1"
MODES = (
...
def build_panels(config: PipelineConfig, *, overwrite: bool = False) -> dict[str, Any]:
    raw = config.processed_trade_dir / "final" / "historical_replication_outcomes.parquet"
    package = policy_root(config) / "package_policy_validation_anchor.parquet"
    independent = policy_root(config) / "historical_tariffs.parquet"
    ...
    con = duckdb.connect()
    clock = policy_root(config) / "historical_event_clock.parquet"
    result: dict[str, Any] = {
        "version": VERSION,
            ...
            if overwrite or not destination.exists():
                if mode == "package_full_policy_anchor":
                    ...
                    SELECT r.*,
                           ...
                           'locked_201_232_301_mfn_paper_clock' AS policy_semantics
...
def _plot(config: PipelineConfig, frames: dict[tuple[str, str, str], pd.DataFrame]) -> dict[str, str]:
    ...
    for spec in SPECS:
        ...
        path = figure_root / f"historical_replication_{spec}.png"
